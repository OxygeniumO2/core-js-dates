...
function getTime(date) {
  ...
  const newDateMinutes =
    newDate.getMinutes() < 10
      ? `0${newDate.getMinutes()}`
      : newDate.getMinutes();
  const newDateSeconds =
    newDate.getSeconds() < 10
      ? `0${newDate.getSeconds()}`
      : newDate.getSeconds();
  return `${newDateHours}:${newDateMinutes}:${newDateSeconds}`;
}
...
function getNextFriday(date) {
  ...
  const myDate = new Date(originalDate);

  for (let i = 1; i < 8; i += 1) {
    myDate.setDate(originalDate.getDate() + i);
...
function getCountDaysInMonth(month, year) {
  ...
  const myDateOneMonthNext = new Date(year, month);
  const millisecondsInOneDay = 1000 * 60 * 60 * 24;
  return Math.round((myDateOneMonthNext - myDate) / millisecondsInOneDay);
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const MyDateEnd = new Date(dateEnd);
  const millisecondsInOneDay = 1000 * 60 * 60 * 24;
  return Math.round((MyDateEnd - MyDateStart) / millisecondsInOneDay) + 1;
}
...
function isDateInPeriod(date, period) {
  ...
  if (endDate - currDate > endDate - startDate || endDate - currDate < 0) {
    return false;
  }
  return true;
}
...
function formatDate(date) {
  const myDate = new Date(date);
  const currMonthDayYear = myDate.toLocaleDateString('en-US', {
    timeZone: 'UTC',
  });
  const currTime = myDate.toLocaleTimeString('en-US', { timeZone: 'UTC' });
  return `${currMonthDayYear}, ${currTime}`;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let i = 1; i <= numberOfDays; i += 1) {
    ...
    if (currDay === 6 || currDay === 0) count += 1;
  }
  return count;
}
...
function getWeekNumberByDate(date) {
  ...
  const firstDayOfTheYear = new Date(currYear, 0, 1);
  const firstWeekStart = firstDayOfTheYear.getDay();
  const millisecondsInDay = 1000 * 60 * 60 * 24;
  const millisecondsInWeek = millisecondsInDay * 7;

  if (firstWeekStart !== 0) {
    firstDayOfTheYear.setDate(
      firstDayOfTheYear.getDate() + (7 - firstWeekStart)
    );
  }

  const diffTime = date - firstDayOfTheYear;
  const weeks = Math.ceil(diffTime / millisecondsInWeek) + 1;
...
function getNextFridayThe13th(date) {
  ...
  const endDate = new Date(9999, 0, 1);
...
function getQuarter(date) {
  const myDate = new Date(date).getMonth();
  if (myDate === 12 || myDate <= 2) {
    return 1;
  }
  if (myDate <= 5) {
    return 2;
